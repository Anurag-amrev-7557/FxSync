...
export default function useResyncAnalytics() {
  ...
  const updateResyncHistory = useCallback(
    (result, drift, message, duration, currentTrackId = 'unknown') => {
      const resyncEntry = {
        timestamp: Date.now(),
        result,
      ...
      setResyncStats((prev) => {
        ...
        const successfulResyncs =
          result === 'success' ? prev.successfulResyncs + 1 : prev.successfulResyncs;

...
function BottomTabBar({ mobileTab, setMobileTab, handleExitRoom }) {
  ...
  return (
    <nav
      ref={containerRef}
      className="fixed bottom-4 left-1/2 transform -translate-x-1/2 inline-flex w-auto bg-neutral-900 backdrop-blur-md border border-neutral-800/70 flex relative z-50 shadow-2xl rounded-full px-2 p-1 gap-1"
    ...
    >
      ...
      >
        ...
        <span className={`text-sm font-medium ${mobileTab === 3 ? 'text-black' : 'text-white'}`}>Spatial</span>
        {mobileTab === 3 && (
          <div className="absolute inset-0 bg-primary/10 rounded-full animate-pulse pointer-events-none"></div>
        )}
      </button>
